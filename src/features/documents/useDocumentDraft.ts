...
import { Money, fromMajor, zero } from '@/lib/money';
...
export function emptyDraft(baseCurrency: string, kind: DocumentKind): DraftState {
  return {
    partyId: null,
    date: today(),
    dueDate: kind === 'invoice' ? addDaysISO(today(), 30) : undefined,
...
export function lineFromItem(item: Item, taxCategories: TaxCategory[]): DocumentLine {
...
export function useDocumentDraft(options: {
  ...
  return { draft, setDraft, patch, addLine, updateLine, removeLine, totals };
}
