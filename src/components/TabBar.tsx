...
const slotOf = (index: number) => (index < 2 ? index : index + 1);
...
/**
 * A floating pill with the four tabs split around Lixi's orb, which sits in a
 ...
 * worth raising.
 */
export function TabBar({ state, descriptors, navigation, lixiNudge = 0 }: TabBarProps & { lixiNudge?: number }) {
  const t = useTheme();
  ...
  useEffect(() => {
    Animated.spring(slide, {
      toValue: slotOf(state.index) * slotWidth,
